...
import { IOrderDocument, IExtendedDelivery, IDeliveredWork, IOrderMessage } from '@mjmakenzi/jobber-shared';
...
class OrderService {
  ...
  async cancelOrder(paymentIntentId: string, orderId: string, body: IOrderMessage): Promise<AxiosResponse> {
    const response: AxiosResponse = await axiosOrderInstance.put(`/cancel/${orderId}`, {
      paymentIntentId,
      orderData: body
    });
    return response;
